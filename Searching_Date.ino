/*
The following code searches for a specific date introduced by the user in a CSV file logged by different sensors. The code checks if 
the date exists and if so it prints out the reading values associated with that date
...
//String mydate = "8/1/2019";
//String myString = String("9/1/2019");
String my_date;
//String &mdate=my_date;
...
// Function that return the introduced date by the user
my_date = readingsdate();

delay(3000);

Serial.println("These are the readings related to the selected date:");
Serial.println("");
  ...
  if (myFile) {
    ...
    // Break a string value into tokens
     ...
     // Convert each character array value into string values to compare it with the introduced value (date) by the user
...
// Introduce the MONTH:
Serial.println("Introduce the MONTH");
...
while ((month_>12) || (month_==0)) {
        ...
        Serial.println("");
        Serial.print("You introduced the month = ");
        Serial.println(month_);
        Serial.println("");
        delay(2000);
...
// Introduce the DAY:
        ...
        Serial.flush();
        Serial.begin(9600);
        Serial.println("Introduce the DAY");
        while (Serial.available()==0);
...
while ((day_ > 31) || (day_ == 0)) {
        ...
        Serial.println("");
        Serial.print("You introduced the DAY = ");
        Serial.println(day_);
        Serial.println("");
        delay(2000);
...
// Introduce the YEAR:
        ...
        Serial.flush();
        Serial.begin(9600);
        Serial.println("Introduce the YEAR");
        while (Serial.available()==0);
...
while ((year_ > 2019) || (year_ < 2015) || (year_ == 0)) {
        ...
        Serial.println("");
        Serial.print("You introduced the YEAR = ");
        Serial.println(year_);
        Serial.println("");
...
Serial.print("The date to search is: ");
